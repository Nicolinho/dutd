...
def load_runs(args):
  ...
  for indir in args.indir:
    ...
    for filename in filenames:
      ...
      if 'plots' in filename.as_posix():
        continue
      method, task_seed = filename.relative_to(indir).parts[:-1]
      if '_' not in task_seed:
        continue
      task, seed = task_seed.rsplit('_', 1)
...
def load_run(filename, indir, args):
  ...
  print(args.yaxis)
  if args.yaxis == 'crossvalimage_loss':
    ys = ys / (64*64)
