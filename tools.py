...
class Logger:
  ...
  def __init__(self, logdir, step, record_videos=True):
    self._logdir = logdir
      ...
      if self._record_videos:
        for name, value in self._videos.items():
          try:
            ...
            print("Error when logging video: ")
            print(e)
    self._writer.flush()
    self._scalars = {}
    self._images = {}
...
def sample_episodes(episodes, length=None, balance=False, seed=0):
  # episodes is a dict (train_eps or eval_eps) which is changed over time,
  # but as it just got the pointer this fct samples from the overtime growing
  # dictionary
...
def determine_num_valid_chunks_cv_dataset(episodes, length):
  ct = 0
  for episode in episodes.values():
    ct += len(episode['image']) // length
  return ct

def sample_episodes_crossval(episodes, length=None, balance=False, seed=0):
  # episodes is a dict (train_eps or eval_eps) which is changed over time,
  # but as it just got the pointer this fct samples from the overtime growing
  # dictionary
  for episode in episodes.values():
    for j in range(0, len(episode['image']) // length):
      yield {k: v[j * length: (j+1) * length] for k, v in episode.items()}
...
def static_scan(fn, inputs, start, reverse=False):
  ...
  # print('shape', tf.nest.flatten(inputs)[0].shape[0]) # for profiler
...
def compute_total_interactions(train_eps, crossval_eps):
  total = 0
  for key, ep in train_eps.items():
    total += len(ep['reward']) - 1
  for key, ep in crossval_eps.items():
    total += len(ep['reward']) - 1
  return total
