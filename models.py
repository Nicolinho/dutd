...
class WorldModel(tools.Module):
  ...
  def train(self, data, only_losses=False):
    data = self.preprocess(data)
    with tf.GradientTape() if not only_losses \
            else dummy_context_mgr() as model_tape:
  ...
  def preprocess(self, obs):
    ...
    obs = obs.copy()
    # images are normalized to [-0.5,0.5] instead of [0,1]
